...
For the minimum occurrence of words, we use 3 (min_count). We already removed stopwords in the clean_doc, so we do not add a stopword vector here. Of course, we choose the encoding "UTF-8". 
...
dtm_aus
```
We have already evaluated k, so we will keep 15 topics here. We now want to choose further hyperparameters.
...
?topicmodels::LDA
set.seed(50)
initial_model <- topicmodels::LDA(dtm_aus, k = 15, method = "Gibbs", control = list(alpha = 0.5, burnin = 100, iter =1000))
...
```{r}
set.seed(50)
initial_model_train <- topicmodels::LDA(dtm_train, k = 15, method = "Gibbs", control = list(alpha = 0.5, burnin = 100, iter =1000))
set.seed(50)
