...
We chose manifestos from Austria from 2017 from polidoc. You can find those in the folder "austria_manifestos"
```{r}
...
Run Wordfish:
We need to rank two texts, we make the justifiable statement that Die Grünen are left of FPÖ.
```{r}
...
```
In this data intolerant, dogmatism, obsession and fanaticism are closely related to ideology.
...
set.seed(50)  #same result if code is run again
model_aus <- topicmodels::LDA(dtm_aus, k = 15, method = "Gibbs", control = list(alpha = 0.5, burnin = 1000, iter =3000)) #burnin and iter will be improved in exerice 3 of chapter 7
...
terms(model_aus, 20) 
```
Topic 15 contains "arbeitslos" meaning unemployed. Topic 10 discusses nature but renewable energy is not highly influencal on the topics. You can go deeper into the sentences and their topics to find more information:
...
We use the corpus on sentence basis to get a dataframe and merge it with the topics:
```{r}
...
A closer look into the sentences shows again that it is difficult to expect a specific topic, here, general environmental and work and work discrimination related issues are discussed.

Now, we want to know out how much the different parties talk about renewable energy and unemployment.We check the length of the topics with nrow and the length of the whole manifesto with the sentences variable in the corpus:
...
```
The unemployment topic is most discussed by KPÖ and SPÖ.