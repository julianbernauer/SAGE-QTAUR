...
#create the plot
ggplot(agg_data, aes(x = name, y = total_sleep, fill = order)) +
...
eli_rel <- as.numeric(dfm_uk_pop2[,"elites"]
/ntoken(dfm_uk))*100
cor_rel <- as.numeric(dfm_uk_pop2[,"corrupt"]
/ntoken(dfm_uk))*100
sov_rel <- as.numeric(dfm_uk_pop2[,"sovereignty"]
/ntoken(dfm_uk))*100
party <- as.character(docvars(corpus_uk,"party"))
...
pop_2 <- ggplot(data = relpop2_sort,
aes(x = nrow(relpop2_sort):1,
y = cor_rel)) +
geom_point() +
coord_flip() +
...
scale_y_continuous(labels =
scales::number_format(accuracy = 0.01))
grid.arrange(pop, pop_1, pop_2, pop_3, nrow = 2)



# KWIC() ANALYSIS
elite_context <- kwic(corpus_uk,
c("elit*", "politici*",
"establishm*", "ruling*"),
window = 10)
...
rap_sent <- dfm_lookup(rap_dfm, dictionary = data_dictionary_
LSD2015)
rap_sent

pos <- as.numeric(rap_sent[,"positive"])
- as.numeric(rap_sent[,"neg_positive"])

neg <- as.numeric(rap_sent[,"negative"])
- as.numeric(rap_sent[,"neg_negative"])
...
rap_lexdiv_ttr <- textstat_lexdiv(rap_dfm,
measure = "TTR")
plot_ttr <- data.frame(rap_lexdiv_ttr,song)
ggplot(plot_ttr, aes(x = reorder(song,
TTR), y = TTR)) +
...
rap_lexdiv_k <- textstat_lexdiv(rap_dfm,
measure = "K")
plot_k <- data.frame(rap_lexdiv_k,song)
ggplot(plot_k, aes(x = reorder(song, K),
y = K)) +
geom_point(color = "blue", size = 2) +
coord_flip() +
labs(x = NULL, y = "Lexical Homogeneity
(Yule’s K)")
...
read_rap <- textstat_readability(rap_corpus,
measure = "ARI")
plot_read <- data.frame(read_rap,song)
ggplot(plot_read, aes(x = reorder(song,
ARI), y = ARI)) +
geom_point(color = "blue", size = 2) +
coord_flip() +
...
plot_comp <- data.frame(net_tone_rap,
TTR,song)
ggplot(plot_comp, aes(x = TTR, y = net_tone_rap)) +
geom_point(colour = "blue", size = 2) +
labs(x = "Lexical Diversity (TTR)",
y = "Net Sentiment (Lexicoder)") +
...
sub_lyr <- dfm_subset(rap_dfm,
sosho=="Message" |
sosho=="Hustlin" |
sosho=="Storm")
sim_cor <- textstat_simil(sub_lyr)
sim_cos <- textstat_simil(sub_lyr,
method = "cosine")
sim_cor
sim_cos

dist_euc <- textstat_dist(sub_lyr)
dist_man <- textstat_dist(sub_lyr,
method = "manhattan")
dist_euc
dist_man
...
ggplot(plot_sim, aes(x = simvec, y = distvec)) +
geom_point(colour = "blue", size = 2) +

geom_smooth(mapping = aes(x = simvec,
y = distvec), method=’lm’,
se = F, linetype = "dashed") +
labs(x = "Cosine Similarities (Pairwise)",
y = "Euclidean Distances (Pairwise)") +
geom_text(aes(label=so),hjust=-0.1,
vjust=-0.1) +
xlim(.45,0.75)
